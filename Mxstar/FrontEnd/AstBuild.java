        ...
        if(ctx.thenStatement.getText().length() == 1) {
            ...
            if (ctx.thenStatement.accept(this).getClass() != BlockStmt.class) {
                ...
                if (ctx.thenStatement.accept(this).getClass() == LinkedList.class) {
                    tmp.location = (((List<Stmt>) ctx.thenStatement.accept(this)).get(0)).location;
                    tmp.statements = ((List<Stmt>) ctx.thenStatement.accept(this));
                } else {
                    tmp.location = ((Stmt) ctx.thenStatement.accept(this)).location;
            ...
            if(ctx.elseStatement.getText().length() == 1) {
                ...
                if (ctx.elseStatement.accept(this).getClass() != BlockStmt.class) {
                    ...
                    if (ctx.elseStatement.accept(this).getClass() == LinkedList.class) {
                        tmp.location = (((List<Stmt>) ctx.elseStatement.accept(this)).get(0)).location;
                        tmp.statements = ((List<Stmt>) ctx.elseStatement.accept(this));
                    } else {
                        tmp.location = ((Stmt) ctx.elseStatement.accept(this)).location;
                        tmp.statements = new LinkedList<>();
        ...
        if(ctx.statement().getText().length() == 1) {
            ...
            if (ctx.statement().accept(this).getClass() != BlockStmt.class) {
                ...
                if (ctx.statement().accept(this).getClass() == LinkedList.class) {
                    tmp.location = (((List<Stmt>) ctx.statement().accept(this)).get(0)).location;
                    tmp.statements = ((List<Stmt>) ctx.statement().accept(this));
                } else {
                    tmp.location = ((Stmt) ctx.statement().accept(this)).location;
                    tmp.statements = new LinkedList<>();
...
//            System.out.println(identifier.location);
    ...
    @Override public Expr visitArrayExpr(ArrayExprContext ctx) {
        ...
        /*if(!(expr.idx.type instanceof PrimitiveType)) {
            errorListener.addError(expr.idx.location, "idx must be integer");
        }*/
...
//        System.out.println(assignExpr.lhs.location + ":" + (assignExpr.lhs instanceof Identifier));
    ...
    public FuncDecl visitFunctionDefinition(FuncDefContext ctx) {
        if(ctx.Identifier() == null) {
            errorListener.addError(new Location(ctx.getStart()),"invalid func name");
        }
        ...
        FuncDecl.location = FuncDecl.returnType.location;
        return FuncDecl;
    }

    @Override public FuncDecl visitConstructDefinition(ConstructDefinitionContext ctx) {
        if(ctx.Identifier() == null) {
            errorListener.addError(new Location(ctx.getStart()),"invalid class name");
        }
        ...
        for (MemDeclContext c: ctx.memDecl()) {
            ...
            if(c.funcDef() != null) {
                ...
                if(tmp.name.equals(ClassDecl.name)) {
                    errorListener.addError(new Location(c), "there shouldn't have return val in construction");
                } else {
            ...
            if(c.constructDefinition() != null) {
                if (!c.constructDefinition().Identifier().getSymbol().getText().equals(ClassDecl.name)) {
                    errorListener.addError(new Location(c), "construct definition without a same name");
                } else {
                    if (ClassDecl.construct != null) {
                        errorListener.addError(new Location(c), "too many construction function");
                    } else {
    ...
    @Override public Type visitVariableTypeBasic(VariableTypeBasicContext ctx) {
        if(ctx.INT() != null) {
            return new PrimitiveTypeNode("int",new Location(ctx));
        } else if (ctx.BOOL() != null) {
            return new PrimitiveTypeNode("bool",new Location(ctx));
        } else {
            return new ClassTypeNode(ctx.getText(),new Location(ctx));
        }
    }
    ...
    public Type visitVariableType(VariableTypeContext ctx) {
        if(ctx.empty().isEmpty()) {
            ...
            ArrayTypeNode arrayType = new ArrayTypeNode(new Location(ctx),ctx.empty().size());
    ...
    @Override public ExprStmt visitExprStmt(ExprStmtContext ctx) {
        return new ExprStmt(new Location(ctx),(Expr) ctx.expr().accept(this));
    }
