...
//            System.out.println(node.rhs.type instanceof  ClassType );
    ...
    public void visit(AssignExpr node) {
        ...
        if (!node.lhs.type.match(node.rhs.type)) {
            errorListener.addError(node.location, "assignment different type");
        }
        if (!node.lhs.modifiable) {
            errorListener.addError(node.location, "left val not able to modify");
        }
        node.modifiable = false;
    }
    ...
    public void visit(AstProgram node) {
        ...
        FuncSymbol functionSymbol = globalSymbolTable.getFunctionSymbol("main");
